...
Model1_theta_hat = solve(t(Model1) %*% Model1) %*% t(Model1) %*% df$y
...
```{r}
Model2 = cbind(df$X4, df$X1^3, df$X3^4, ones, noise)
Model2_theta_hat = solve(t(Model2) %*% Model2) %*% t(Model2) %*% df$y
...
```{r}
Model3 = cbind(df$X3^3, df$X3^4, ones, noise)
Model3_theta_hat = solve(t(Model3) %*% Model3) %*% t(Model3) %*% df$y
...
```{r}
Model4 = cbind(df$X2, df$X1^3, df$X3^4, ones, noise)
Model4_theta_hat = solve(t(Model4) %*% Model4) %*% t(Model4) %*% df$y
...
```{r}
Model5 = cbind(df$X4, df$X1^2, df$X1^3, df$X3^4, ones, noise)
Model5_theta_hat = solve(t(Model5) %*% Model5) %*% t(Model5) %*% df$y
...
```{r}
RSS_Model1 = sum((df$y-y_Hat_Model1)^2)
print(RSS_Model1)


RSS_Model2 = sum((df$y-y_Hat_Model2)^2)
print(RSS_Model2)


RSS_Model3 = sum((df$y-y_Hat_Model3)^2)
print(RSS_Model3)


RSS_Model4 = sum((df$y-y_Hat_Model4)^2)
print(RSS_Model4)

RSS_Model5 = sum((df$y-y_Hat_Model5)^2)
print(RSS_Model5)
```
